...
OUT_DIR = os.environ.get('END_OUT_DIR') or os.path.join(ROOT, 'source', 'images')
...
def neko_head_base():
    ...
    sc = HEAD_W / float(x1 - x0)
    pad = HEAD_PAD
    tw, th = HEAD_W, int(round((y1 - y0) * sc))
    ...
    cl = shrink(grow(outer, 3), 3)
    outer[34:] = cl[34:]
    op = dgrow(~dgrow(~outer, 9.0), 9.0)
    outer[50:] = op[50:]
    ...
    ink |= grey & inner & NV.pat(8, h, w)
    ink[17:24, 29:45] = False
    outer[17:24, 29:45] = False
    sx, sy = 27, 20
    for j, r in enumerate(TUFT):
        for i, ch in enumerate(r):
            if ch == '#':
                ink[sy + j, sx + i] = True
                outer[sy + j, sx + i] = True
    for i in range(len(TUFT[0])):
        col = [j for j, r in enumerate(TUFT) if r[i] == '#']
        for yy in range(sy + max(col) + 1, 25):
            if not ink[yy, sx + i]:
                outer[yy, sx + i] = True
    ink[18:22, 45:47] = True
    inner2 = outer & ~ink
    yy, xx = np.mgrid[0:h, 0:w]
    shade = edge_side(inner2, 1, 0, 2) & (yy > 24) & (yy < h - 12)
    ink |= shade & NV.pat(8, h, w)
    ...
    for wy in (129, 138):
        a, yv = P(135, wy)
        right = np.nonzero(outer[yv])[0].max()
        wr.append((a, right - 3, yv))
    (lx, ly), (rx, ry) = P(*NV.EYES_REF[0]), P(*NV.EYES_REF[1])
    ey = (ly + ry) // 2
    mid = (lx + rx) // 2
    half = max(1, (rx - lx) // 2)
    mx, my = P(92.5, 139.5)
    d = dict(ink=ink, outer=outer | ink, w=w, h=h, eyes=((mid - half, ey), (mid + half, ey)),
             mouth=(mid, my), wl=wl, wr=wr, tuft=(sx, sy, len(TUFT[0])))
    _HEAD['h'] = d
    return d


def part(s, m, shade=True):
    s.put(dgrow(m, 2.0) & ~m, True)
    s.put(m, False)
    if shade:
        s.put(edge_side(m, 1, 0, 2) & (s.ys > 0), bayer(8))


def paw(s, x, y, r=7.0, ang=0.0, toes=True):
    m = s.ellipse_mask(x, y, r, r * 0.9)
    s.put(dgrow(m, 2.0) & ~m, True)
    s.put(m, False)
    if toes:
        for k in (-1, 1):
            px = x + k * r * 0.33
            s.put(s.rect_mask(px, y - r + 1, px + 1, y - r + 4) & m, True)
    return m


def paste_head(s, hd, hx0, hy0):
    hm = np.zeros((s.h, s.w), bool)
    hi = np.zeros((s.h, s.w), bool)
    y0c, x0c = max(0, hy0), max(0, hx0)
    sub_o = hd['outer'][y0c - hy0:, x0c - hx0:][:s.h - y0c, :s.w - x0c]
    sub_i = hd['ink'][y0c - hy0:, x0c - hx0:][:s.h - y0c, :s.w - x0c]
    hm[y0c:y0c + sub_o.shape[0], x0c:x0c + sub_o.shape[1]] = sub_o
    hi[y0c:y0c + sub_i.shape[0], x0c:x0c + sub_i.shape[1]] = sub_i
    s.put(hm, False)
    s.put(hi, True)
    return hm


def neko_figure(s, cx, feet_y, arms='up', tail=True, extra=None):
    hd = neko_head_base()
    hw, hh = hd['w'], hd['h']
    hx0 = int(round(cx - hw / 2))
    hy0 = feet_y - 22 - (hh - HEAD_PAD)
    if tail:
        tp = [(cx + 18, feet_y - 7), (cx + 34, feet_y - 5), (cx + 43, feet_y - 13), (cx + 41, feet_y - 24), (cx + 35, feet_y - 28)]
        tm = s.mask(lambda d: d.line(tp, fill=1, width=8, joint='curve')) | s.disc_mask(tp[-1][0], tp[-1][1], 4)
        tm |= s.disc_mask(tp[1][0], tp[1][1], 4) | s.disc_mask(tp[2][0], tp[2][1], 4)
        part(s, tm)
    body = s.ellipse_mask(cx, feet_y - 20, 25, 18)
    if arms == 'up':
        specs = [((cx - 22, feet_y - 28), (cx - 42, feet_y - 44), (cx - 47, feet_y - 70)),
                 ((cx + 22, feet_y - 28), (cx + 43, feet_y - 46), (cx + 48, feet_y - 72))]
    else:
        specs = [((cx - 20, feet_y - 30), (cx - 34, feet_y - 24), (cx - 42, feet_y - 16)),
                 ((cx + 20, feet_y - 32), (cx + 44, feet_y - 42), (cx + 50, feet_y - 64))]
    arm_ms = []
    for sh, el, pw in specs:
        am = capsule(s, sh, el, 7.0) | capsule(s, el, pw, 6.5)
        arm_ms.append(am)
        part(s, am)
    part(s, body)
    for side in (-1, 1):
        fx = cx + side * 12
        fm = s.ellipse_mask(fx, feet_y - 4, 9.5, 5.5)
        part(s, fm, shade=False)
        for k in (-2.5, 2.5):
            s.line([(fx + k, feet_y - 1), (fx + k, feet_y - 3)], 1, True)
    hm = paste_head(s, hd, hx0, hy0)
    seam = hm & shrink(body, 2) & (s.ys > hy0 + hh - HEAD_PAD - 5)
    s.put(seam, False)
    s.put(seam & edge_side(body, 1, 0, 4) & ~edge_side(body, 1, 0, 2), False)
    armu = np.zeros((s.h, s.w), bool)
    for am in arm_ms:
        armu |= dgrow(am, 2.0)
    for a, b, yv in hd['wl']:
        s.put(s.rect_mask(hx0 + a, hy0 + yv, hx0 + b + 1, hy0 + yv + 1) & (hm | ~armu), True)
    for a, b, yv in hd['wr']:
        s.put(s.rect_mask(hx0 + a, hy0 + yv, hx0 + b, hy0 + yv + 1), True)
    if extra:
        extra(s, specs)
    for sh, el, pw in specs:
        paw(s, pw[0], pw[1], 8.5 if arms == 'up' else 7.0)
    (ex0, ey0), (ex1, ey1) = hd['eyes']
    mx, my = hd['mouth']
    return dict(hx0=hx0, hy0=hy0, head=hm, eyes=((hx0 + ex0, hy0 + ey0), (hx0 + ex1, hy0 + ey1)),
                mouth=(hx0 + mx, hy0 + my), body=body, paws=[p for _, _, p in specs],
                top=hy0 + HEAD_PAD + 4, tuft=(hx0 + hd['tuft'][0], hy0 + hd['tuft'][1], hd['tuft'][2]))


def face_shocked(s, f):
    for (x, y) in f['eyes']:
        s.ellipse(x, y, 7.2, 7.8, True)
        s.ellipse(x, y, 5.1, 5.7, False)
        s.disc(x, y + 0.5, 2.2, True)
    mx, my = f['mouth']
    s.ellipse(mx, my + 1, 2.6, 3.2, True)
    s.ellipse(mx, my + 1.4, 1.1, 1.5, False)


def face_dizzy(s, f):
    for i, (x, y) in enumerate(f['eyes']):
        s.ellipse(x, y, 7.5, 7.5, False)
        side = -1 if i == 0 else 1
        pts = []
        for k in range(60):
            t = k / 59 * math.tau * 2.3
            r = 0.3 + k / 59 * 6.6
            pts.append((x + math.cos(t * side) * r, y + math.sin(t * side) * r))
        s.line(pts, 1, True)
    mx, my = f['mouth']
    s.ellipse(mx, my + 1, 3.2, 2.6, True)
    s.ellipse(mx, my + 1.2, 1.6, 1.1, False)


def char_neko(s, f):
    fur = s.alpha & ~s.ink
    fur = shrink(fur, 1)
    (elx, ely), (erx, ery) = f['eyes']
    mx, my = f['mouth']
    keep = s.ellipse_mask(elx, ely, 9, 9) | s.ellipse_mask(erx, ery, 9, 9) | s.ellipse_mask(mx, my + 1, 6, 5)
    hx0, hy0 = f['hx0'], f['hy0']
    rng = np.random.default_rng(12)
    blots = [(elx - 8, ely - 20, 11, 7, 0.3), (erx + 12, ery + 12, 8, 6, -0.4), (mx + 14, my + 26, 10, 6, 0.1),
             (hx0 + 22, ely + 14, 5, 6, 0.5), (erx + 4, hy0 + 20, 6, 4, 0.0), (f['paws'][0][0] + 3, f['paws'][0][1] - 8, 5, 4, 0.2),
             (mx + 34, my + 4, 5, 6, 0.0)]
    for bx, by, rx, ry, a in blots:
        ang = np.arctan2(s.ys + 0.5 - by, s.xs + 0.5 - bx) - a
        wob = 1 + 0.22 * np.sin(ang * 5 + rng.uniform(0, 6)) + 0.12 * np.sin(ang * 9 + rng.uniform(0, 6))
        d = np.hypot((s.xs + 0.5 - bx) / rx, (s.ys + 0.5 - by) / ry) / wob
        core = (d < 0.75) & fur & ~keep
        rim = (d >= 0.75) & (d < 1.0) & fur & ~keep
        s.put(core, bayer(13))
        s.put(rim, bayer(7))
    tx, ty, tw = f['tuft']
    s.put(s.rect_mask(tx + 2, ty - 1, tx + tw - 2, ty + 3) & ~s.ink, False)
    strands = [(-12, -0.4, 10), (-5, -0.12, 13), (2, 0.12, 11), (9, 0.4, 12), (26, 0.75, 8), (-27, -0.7, 7)]
    head = f['head']
    paths = []
    cx0 = tx + tw // 2
    for k, (dx, a, L0) in enumerate(strands):
        bx = cx0 + dx
        col = np.nonzero(head[:, bx])[0]
        if not len(col):
            continue
        by = col.min() + 2
        pts = [(bx, by)]
        for i in range(1, 5):
            t = i / 4
            w = 1.3 if i % 2 else -1.3
            pts.append((bx + math.sin(a) * L0 * t + w * math.cos(a), by - math.cos(a) * L0 * t + w * math.sin(a)))
        paths.append(pts)
    for pts in paths:
        s.line(pts, 4, False)
    for pts in paths:
...
def exclaim(c, x, y, h=11, ang=0.0):
    top = (x - math.sin(ang) * h, y - math.cos(ang) * h)
    m = capsule(c, (x, y - 5), top, 2.3)
    m |= capsule(c, (x - math.sin(ang) * 2, y - 7), (top[0] + 0.0, top[1]), 2.8) & (c.ys < y - 6)
    dot = c.disc_mask(x + math.sin(ang) * 1.5, y + 1.5, 2.4)
    c.put(dgrow(m | dot, 2.0) & ~(m | dot), False)
    c.put(m | dot, True)


def dist_map(m, r):
    dist = np.full(m.shape, 99.0)
    g = m.copy()
    dist[g] = 0
...
def light_pool(c, fig_alpha, r0=2, r1=11):
    dist = dist_map(fig_alpha, r1)
    zone = (dist > 0) & (dist <= r1)
    keep = ~tone(np.clip(1.0 - (dist - r0) / float(r1 - r0), 0, 1))(c.xs, c.ys)
    c.ink[zone] &= keep[zone]
    c.ink[(dist > 0) & (dist <= r0)] = False


def smoke_pool(c, fig_alpha, r0=2, r1=14, level=6, fade=5):
    dist = dist_map(fig_alpha, r1)
    zone = (dist > r0) & (dist <= r1)
    calm = bayer(level)(c.xs, c.ys)
    t = np.clip((dist - (r1 - fade)) / float(fade), 0, 1)
    pick_old = tone(t)(c.xs, c.ys)
    new = np.where(pick_old, c.ink, calm)
    c.ink[zone] = new[zone]
    c.ink[(dist > 0) & (dist <= r0)] = False
...
def scene_caught():
    ...
    rob = layer()
    nfig = NB.render('hands_up', width=96, face_props=False)
    NB.place(rob, nfig, rcx, feet_y, halo=0)
    ...
    shadow = shift(rob.alpha, 12, 4) & (c.ys < floor_y - 7) & ~grow(rob.alpha, 2)
    lit.put(shadow, bayer(13))
    ...
    NB.halo_rim(c, rob.alpha)
    comp(c, rob)
    htop = feet_y - nfig['G'] + nfig['head_top']
    exclaim(c, rcx + 64, htop + 30, 15, -0.2)
    exclaim(c, rcx + 74, htop + 38, 12, -0.35)
    sweat_drop(c, rcx - 60, htop + 20, 1.5, -0.6)
    sweat_drop(c, rcx + 58, htop + 58, 1.2, 0.6)
...
def scene_boom():
    ...
    shape = (c.ellipse_mask(214, 44, 58, 34) | c.ellipse_mask(212, 104, 62, 48) | c.ellipse_mask(196, 150, 40, 30)
             | c.ellipse_mask(236, 150, 44, 34) | c.ellipse_mask(256, 90, 26, 34))
    ...
    for k, (x, y, a, sg) in enumerate(((150, 12, 0.5, 0.0), (64, 128, -0.4, 0.4), (60, 20, 0.9, 0.0), (22, 182, 0.3, 0.0), (44, 56, -0.8, 0.5), (106, 12, -0.2, 0.45), (290, 18, 0.7, 0.0), (322, 50, -0.3, 0.4), (196, 196, 1.1, 0.0))):
        cash_bill(c, x, y, a, singe=sg)
    alarm_bell(c, 364, 30)
    rob = layer()
    rcx, feet_y = 96, 238
    u = 96
    nfig = NB.render('dazed', width=u, soot=[(-0.16, 0.2, 0.08, 0.05), (0.2, 0.44, 0.07, 0.045), (-0.36, 0.34, 0.06, 0.045), (0.34, 0.16, 0.05, 0.04)])
    NB.backdrop(c, nfig, rcx, feet_y, 0.78, 6, 20)
    c.put(c.ellipse_mask(rcx + 4, feet_y - 1, 58, 5), bayer(13))
    NB.place(rob, nfig, rcx, feet_y, halo=0)
    NB.halo_rim(c, rob.alpha)
    comp(c, rob)
    J = feet_y - nfig['G'] + nfig['J']
    top = J - int(0.9 * u)
    for (x, y) in ((rcx - 58, top + 18), (rcx + 56, top + 6), (rcx - 40, top - 4)):
        tiny_star(c, x, y, 5.5)
    wisp(c, rcx + 44, top + 22, 16, 3, 2.0, 2)
    wisp(c, rcx - 48, top + 40, 14, 3, 0.0, 2)
    NB.repaste(c)
    return c
...
def main():
    ...
    for name, fn in (('end-caught', scene_caught), ('end-boom', scene_boom)):
        ...
        c.preview(os.path.join(OUT_DIR, name + '@3x.png'), 3)
        if PREVIEW_DIR:
            g = Canvas(W, H, 0)
            g.ink = c.ink.copy()
            ...
            g.preview(os.path.join(PREVIEW_DIR, name + '-guide@3x.png'), 3)
