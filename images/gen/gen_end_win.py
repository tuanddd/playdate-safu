...
from pdart import (Canvas, bayer, bayer8, tone, hlines, vlines, hatch, hatch_r, cross, dots, brick,
                   wood, brushed, OR, AND, NOT, rivet, screw, panel, dilate, erode, load_font)
...
def scene_win():
    ...
    RX, RY = 116, 96
    armL = [(RX - 22, RY + 40), (RX - 42, RY + 18), (RX - 48, RY - 12)]
    armR = [(RX + 22, RY + 40), (RX + 42, RY + 18), (RX + 50, RY - 10)]
    sil = c.ellipse_mask(RX, RY, 23, 28) | c.ellipse_mask(RX, RY - 9, 21, 20)
    sil |= c.poly_mask([(RX - 28, RY + 30), (RX + 28, RY + 30), (RX + 32, 200), (RX - 32, 200)])
    for pts in (armL, armR):
        sil |= c.mask(lambda d, pts=pts: d.line(pts, fill=1, width=12, joint='curve'))
    sil |= c.disc_mask(RX - 48, RY - 14, 8) | c.disc_mask(RX + 50, RY - 12, 8)
    c.put(dilate(dilate(sil)), False)
    arm(c, armL, 10, 6, RY + 30)
    arm(c, armR, 10, 6, RY + 30)
    tm = torso(c, RX, RY + 26, 26, 30, 206, 6)
    strap = c.mask(lambda d: d.line([(RX - 22, RY + 34), (RX + 30, RY + 92)], fill=1, width=7)) & tm
    c.put(dilate(strap) & tm, True)
    c.put(strap, bayer(5))
    c.put(strap & ~shift(strap, 1, 1), False)
    c.rect(RX + 2, RY + 58, RX + 12, RY + 66, True)
    c.rect(RX + 4, RY + 60, RX + 10, RY + 64, bayer(2))
    for bxx, byy, a in [(RX - 57, RY - 26, -0.8), (RX - 45, RY - 31, -0.2)]:
        bill(c, bxx, byy, a)
    for bxx, byy, a in [(RX + 48, RY - 29, 0.1), (RX + 60, RY - 24, 0.7)]:
        bill(c, bxx, byy, a)
    white_hand(c, RX - 48, RY - 12, 1.0, -1, -0.35, 0.22)
    white_hand(c, RX + 50, RY - 10, 1.0, 1, 0.35, 0.22)
    robber_head(c, RX, RY, 21, 26, 'joy')
    shake_lines(c, RX - 48, RY - 14, 18, 26, [-2.7, -2.25, -1.85], 2, True)
    shake_lines(c, RX + 50, RY - 12, 18, 26, [-1.3, -0.9, -0.5], 2, True)
    ...
    for bxx, byy, a in [(70, 68, 0.5), (170, 64, -0.6), (206, 104, 0.2), (66, 32, -0.3), (192, 144, 0.9),
                        (62, 122, -1.1), (158, 28, 0.3)]:
        bill(c, bxx, byy, a, 18, 9)
    for sx, sy, r in [(88, 52, 6), (144, 70, 4), (218, 64, 5), (196, 186, 6), (164, 184, 4), (118, 202, 7),
                      (40, 188, 4), (172, 116, 3), (80, 184, 4), (220, 204, 3), (136, 230, 3), (60, 232, 4)]:
...
def scene_timeup():
    ...
    tri = c.poly_mask([(VX, 58), (VX + 14, 82), (VX - 14, 82)])
    c.put(dilate(dilate(tri)), True)
    c.put(tri, False)
    c.put(tri & ~shift(tri, -1, -1) & (c.ys > 78), bayer(6))
    c.rect(VX - 1, 66, VX + 2, 75, True)
    c.rect(VX - 1, 77, VX + 2, 80, True)
    for hx in (VX - 10, VX + 10):
        c.line([(hx, 52), (hx + (4 if hx > VX else -4), 66)], 1, True)
    ...
    bell(c, BX, BY, 19)
    for a, r0, r1 in [(-2.4, 26, 36), (-1.95, 27, 38), (-1.45, 28, 37), (-0.55, 27, 38), (-0.1, 28, 40),
                      (0.35, 27, 36), (2.55, 26, 35), (2.95, 27, 35)]:
        p0 = (BX + math.cos(a) * r0, BY + math.sin(a) * r0)
        p1 = (BX + math.cos(a) * r1, BY + math.sin(a) * r1)
        c.line([p0, p1], 4, True)
        c.line([p0, p1], 2, False)
    for k in range(2):
        arc = c.ring_mask(BX + 1, BY, 46 + k * 7, 47.5 + k * 7) & (c.xs > BX + 30) & (np.abs(c.ys - BY) < 14 + k * 5)
        c.put(dilate(arc), True)
        c.put(arc, False)

    RX, RY = 64, 188
    torso(c, RX, RY + 34, 34, 42, 250, 6, halo=True)
    armL = [(RX - 28, RY + 46), (RX - 50, RY + 42), (RX - 36, RY + 22)]
    armR = [(RX + 28, RY + 46), (RX + 50, RY + 42), (RX + 36, RY + 22)]
    for pts in (armL, armR):
        o = c.mask(lambda d, pts=pts: d.line(pts, fill=1, width=14, joint='curve'))
        c.put(dilate(o) & ~o, False)
    arm(c, armL, 11, 6, RY + 30)
    arm(c, armR, 11, 6, RY + 30)
    robber_head(c, RX, RY, 29, 35, 'panic', halo=True, look=(-1, -2))
    white_hand(c, RX - 31, RY + 14, 1.2, -1, 0.1, 0.2)
    white_hand(c, RX + 31, RY + 14, 1.2, 1, -0.1, 0.2)
    for sx, sy, s_ in [(RX - 36, RY - 32, 1.4), (RX + 36, RY - 36, 1.2), (RX + 44, RY - 12, 1.0),
                       (RX - 44, RY - 10, 1.0)]:
        sweat(c, sx, sy, s_)
    for a in [-2.85, -2.55, -0.59, -0.29]:
        p0 = (RX + math.cos(a) * 52, RY - 14 + math.sin(a) * 52)
        p1 = (RX + math.cos(a) * 62, RY - 14 + math.sin(a) * 62)
        c.line([p0, p1], 4, True)
        c.line([p0, p1], 2, False)
    return c
...
def main():
    ...
    if PREVIEW:
        for name, cv in [('win', win), ('timeup', tu)]:
            cv.preview(os.path.join(PREVIEW, f'end-{name}@3x.png'), 3)
            g = Canvas(W, H, 0)
            g.ink = cv.ink.copy()
            g.put(g.rect_mask(40, 6, 360, 62) & (((g.xs + g.ys) % 8) < 2), False)
            g.rect(230, 66, 398, 238, True)
            g.rect(233, 69, 395, 235, False)
            g.preview(os.path.join(PREVIEW, f'end-{name}-guide@3x.png'), 2)
