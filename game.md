...
### Title
Full-screen plate `source/images/title-neko.png` (`Art.titleNeko`, made by
`images/gen/gen_neko_v2_title.py`), blitted at 0,0. It stars **Neko**, the mascot, listening at
the vault with a heart "CLICK!" bubble. The plate carries a dark brick wall, the steel vault door
with a white cut-out (r=72 at 200,126) for the live dial, and a hand-built pixel **SAFU**
wordmark baked in at y 0–54. So the code draws only the rotating dial (r=62 at 200,126) and the
two calls to action, baked once into `Art.ctaRow` from the `chip.png` nine-slice. If
`title-neko` is missing, the old `title-screen-bg.png` plate and the Bouncy `SAFU` word are used
instead. The crank already ticks here, so the mechanism is alive before the run starts.
...
The old plate (still in `source/images/title-screen-bg.png` as the fallback) is a brick wall with
a round vault door set into it — stone jamb, voussoir joints, 16 bolts, dithered door face —
standing on a receding floor, with two masked robbers at the left and right edges. The dial, wordmark and CTA are cut out of it as white, following
...
The whole screen is the safe door: one baked 1-bit plate, `source/images/play-bg.png`
(`Art.playBg`), generated by `images/gen/gen_play.py`. It has brushed steel, an 8px jamb with
bolts, panel seams, rivet columns, a 4-spoke handle wheel at (37,185), and a keyhole at
(199,210). It also carries the fixed chrome the code used to draw: a shaded bezel ring (r 64–78)
with 12 bolts and a fixed **index mark** at the top of the dial, a recessed timer bezel, the dark
recessed channel behind the modifier plaques (x 216–386), and a black prompt plate under the
dial (x 62–182, y 182–223). If the PNG is missing, `Art.drawDoor` falls back to the old
hand-drawn door.
...
Plates are the nine-slice `source/images/plaque.png` (`Art.plaque`, 12px corners, 3px shadow
included). Each has a white face, a 2px border, and a slotted screw in each corner.
`Art.drawPlate` draws it at (w+3, h+3). Dialogs use `panel.png` (`Art.panel`, 16px corners, 4px
shadow). Small black labels use `chip.png` (`Art.chip`, 6px corners, 2px shadow), which draws the
title CTAs and the docked notice. Without the PNGs, the old scooped polygon (`notchedPoly`) is
drawn instead.

Each card is two rows. The first is the **title tag** (`Art.drawTag`): the icon and the name in
black Nontendo-Bold, left-aligned with no background, then a dotted leader (`Art.drawLeader`) to
the right edge. The second is
the subtitle, wrapped to **at most 2 lines**. The pause menu's modifier catalogue uses the same
tags. The prompts `↓ OPEN?` and `Ⓑ MENU` are drawn in white on the baked prompt plate. Title and subtitle are the same family at two sizes and weights —
...
3. Panel: an illustrated scene, `source/images/end-win.png`. The vault is open, and Neko
   hugs a $ coin, happy-faced, with a heart bubble and loot. It is made by
   `images/gen/gen_neko_v2_end_win.py`. `SAFE OPEN!` is across
   the top. A report card (`Report.*` in `main.lua`: `Art.panel` at (234,64), 156×172) reads:
   `TIME LEFT` tag and frozen `mm:ss.cc` (or `TUTORIAL` / `n OF 2`), then a `CRACKED WITH` tag
...
3. Panel: one illustrated scene per cause, each starring Neko:
   - `end-timeup.png`: the alarm rings and the gate comes down. Neko stands panicking, paws on
     cheeks.
   - `end-caught.png`: Neko has its hands up in the guard's flashlight beam.
   - `end-boom.png`: Neko sits dazed and charred in front of the blown vault, holding the
     cracked flask.

   Timeup comes from `images/gen/gen_neko_v2_end_win.py`, the other two from
   `gen_neko_v2_end_lose.py`. The SFX word sits across the
   top. The same report card follows: a `TUMBLERS FOUND` tag, the progress dots, a `PLAYED WITH`
   tag, the modifier rows, then `Ⓐ TRY AGAIN` / `Ⓑ TITLE`. The chip-row and black-panel layout
   described below is the fallback when a scene PNG is missing.
...
1-bit as an intentional aesthetic: mechanical, high contrast, slight manga influence. The level of
detail follows the Crank Lab mockups. Surfaces are textured with Bayer levels, hatching and
grooves. Every object has a black outline, a 1px white highlight on its lit edge, dithered
shading on its shadow side, and a dithered drop shadow. **The dial is still the visual star**,
set into a bezel on a detailed vault door.

**Neko, the mascot.** A white cat drawn in duo tone: bold 2px black outline, white fill, and a 50%
checker for grey (inner ears, the shadow band under the jaw, the right-side shading). Its face is
the one in `art-references/wagmi-neko-5.png`: solid black oval eyes, a `:3` mouth, two whiskers
each side, and a fur zigzag between the ears. Its body is the maneki-neko toy build
(`art-references/neko-body2-*.png`): the head is about 59% of standing height with no neck, and
it has an upright capsule torso and short, thick legs whose soles show paw pads. Its arms are
short stick arms, about 0.25 head-width long, with two toe lines at the tip. There is no collar,
no bell and no belly coin.

**The body is one continuous silhouette.** Torso, arms, legs and tail are merged into a single
mask with smoothed joints, like a vinyl toy. One 2px outline is drawn around the whole shape, and
inner lines appear only as short 2px caps where an arm lies against the body, plus the foot soles
when they face the viewer.

**Shading uses offset light.** The shade is the silhouette minus a copy of itself shifted 5px
left and 3px up. That gives a crescent on the right and bottom that fades out in concave areas.
It uses one global checker phase. The jaw shadow is the head's own bottom contour projected onto
the body, tapering at both ends.

**Placing Neko in a scene.** Scenes put a 2px white halo and then a 3px solid black rim around
Neko, grown outward from the silhouette. They paste the sprite last, as one opaque unit, so no
decoration can break its outline. The check script `verify_scenes.py` in the scratchpad confirms
zero white pixels on the outer outline.

`images/gen/neko_body.py` draws the poses (stand, cheer, panic,
hands_up, sit, dazed, rich) on the exact 2x head. `gen_neko_v2_expr.py` holds the 32 expression
stamps. Review sheets and rejected rounds live in `prototypes/neko/v2/`.

**Art pipeline.** Static art is generated, never hand-edited. It lives in `images/gen/`:
`pdart.py` holds the shared primitives (a numpy 1-bit canvas, Bayer 4×4/8×8, pattern fills,
rivets, screws, panels). `gen_play.py`, `gen_end_win.py` and `gen_end_lose.py` write their PNGs
into `source/images/`. Run them from the repo root with a Python that has Pillow and NumPy.
`Art.*` loads each asset only if its `.pdi` exists and falls back to the old code-drawn look
otherwise.

**The timer** has no plate of its own. The clock icon and the digits are black, centred in the
white window of the baked bezel (x 22–120, y 18–46). `Art.drawTimerPlate` re-blits just the
bezel region of `play-bg`, so BLACKOUT's dark room still shows a readable clock.

**The dial** (`Art.drawDial`) adds cheap shading on top of the old drawing: a light-dithered inner
disc, a dithered inner-shadow arc on the lower rim, a white highlight arc on the knurled rim, and a
dithered chrome hub with its own highlight. All of these are native fills and arcs, so the per-frame
cost barely changes.
...
## 13. Code map
    ...
    source/
      ...
      images/play-bg.png         the baked vault door (play screen)
      images/plaque.png, panel.png, chip.png   nine-slices for cards, dialogs, label chips
      images/end-win|timeup|caught|boom.png    the end-screen scenes
...
**Current launcher art is rendered from the Neko title in the game itself.** The title frame is
captured with the simulator autopilot. `launchImage.png` is that frame thresholded to 1-bit.
`card.png` is its top 400×177 crop scaled to 350×155 with `-filter Point`. `icon.png` is Neko's
28px head (`prototypes/neko/v2/neko-v2-head-28.png`) centred on 32×32. The robber-era launcher
files are kept in `prototypes/neko/v2/_replaced-launcher/`. The notes below describe how that
older art was made.
...
**Numerals are cached per angle.** Each of the ten dial numerals is rotated once per dial unit
(100 angles), lazily, into `numRot`, then blitted with `drawCentered`. This replaces ten
`drawRotated` calls per frame. The saving pays for the dial's added shading, so the UI overhaul
costs no frame time. Title CTAs are baked once into `Art.ctaRow` for the same reason.
